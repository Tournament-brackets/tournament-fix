...
function loser_bracket_html(byes, rounds, seed, slots, pos) {
    ...
    if (rounds > 0) {
        ...
        var $placeholder = $("<tr class='matchup-member loser-bracket-member'><td class='seed'></td><td class='name'></td><td class='advance-competitor'><button><span class='glyphicon glyphicon-arrow-right'></span></button></td></tr>"),
        ...
        $winner.append($placeholder.clone().attr("data-loser", loser_game).attr("data-game", game_number + (slot_count / 2)).html("<td class='seed'></td><td class='name'>Loser of " + loser_game + "</td><td class='advance-competitor'><button><span class='glyphicon glyphicon-arrow-right'></span></button></td>"));
        $winner.append($placeholder.clone().attr("data-winner", game_number).attr("data-game", game_number + (slot_count / 2)));
        ...
        if (rounds === 1) {
            ...
            if (seed_inv <= byes) {
                ...
                $winner.append($placeholder.clone().addClass("bye").attr("data-loser", loser_game).attr("data-game", game_number + (slot_count / 2)).html("<td class='seed'></td><td class='name'>Loser</td>"));
                $winner.append($placeholder.clone().addClass("bye").html("<td class='seed'></td><td class='name'>Bye</td>"));
            }
            if (seed > byes) {
                $matchups.append($placeholder.clone().attr("data-loser", round_base - slot_count * 2 + seed).attr("data-game", game_number));
                $matchups.append($placeholder.clone().attr("data-loser", round_base - slot_count * 2 + seed_inv).attr("data-game", game_number));
            } else {
                $matchups.addClass("bye");
                $matchups.append($placeholder.clone().addClass("bye").attr("data-loser", round_base - slot_count * 2 + seed_inv).attr("data-game", game_number));
            ...
            slots = new_slots;
            $matchups.append($placeholder.clone().attr("data-game", game_number).attr("data-winner", dependent_base + seed));
            $matchups.append($placeholder.clone().attr("data-game", game_number).attr("data-winner", dependent_base + seed_inv));
            $bracket.find(".winner").children(".connect-wrap").append($winner).append(bracket_connector(-1, 1, game_number));
...
function bracket_html(competitors, rounds, seed, slots, tourney_type) {
    ...
    var seed_inv = slots - seed + 1,
        ...
        $placeholder = $("<tr data-winner='" + (dependent_base + seed) + "' data-game='" + game_number + "'class='matchup-member'><td class='seed'></td><td class='name'></td><td class='advance-competitor'><button><span class='glyphicon glyphicon-arrow-right'></span></button></td></tr>"),
        $placeholder2 = $placeholder.clone().attr("data-winner", dependent_base + seed_inv),
    ...
    $bracket = $table.clone();
    if (slots === 2) {
        $bracket.find(".winner").addClass("champion").children(".connect-wrap").append($("<table class='matchup-group'></table").append($placeholder.clone().removeAttr("data-winner").removeAttr("data-game"))).find(".connect").replaceWith(bracket_connector(0, 1));
        ...
        if (tourney_type == 2) {
            ...
            $bracket.find(".matchup").after($("<td class='winner round-" + (rounds + 2) + "'></td>").append($("<div class='connect-wrap'></div>").append($("<table class='matchup-group'></table").append($placeholder.clone()).append($placeholder2.clone().attr("data-game", round_base * 2 - 1).attr("data-winner", round_base * 2 - 2))).append(bracket_connector(0, 2))));
            $placeholder.attr("data-game", round_base * 2).removeAttr("data-winner");
            $bracket.find(".winner.champion").before($("<td class='winner if-necessary round-" + (rounds + 2) + "'></td>").append($("<div class='connect-wrap'></div>").append($("<table class='matchup-group'></table").append($placeholder.clone().attr("data-loser", round_base * 2 - 1)).append($placeholder.clone().attr("data-winner", round_base * 2 - 1))).append(bracket_connector(0, 2))));
        }
        for (i = 1; i <= rounds + 1; i++) {
...
function build_bracket(tourney_type, tourney_seeds, competitors) {
    'use strict';
